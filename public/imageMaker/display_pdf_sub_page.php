...
// this file generates a sub header menu for a top level folder, for folders with no image folders (i.e. pdfs) e.g. Documents
// allows for nested menus, tabs, etc
            ...
            <ul class="nav navbar-nav nav-flex-icons" style="color:white">
                ...
                <?php
					...
					foreach($rootFolders as $rootFolder) { // top level menu items				
						...
						foreach ($subFolders as $subFolder) { // sub menu items
		
							// NOTE: folderDepth = the number of folders including the subFolder
							$folderDepth = 1;
							...
							if (($folderDepth > 1 && !$tabs) || ($folderDepth == 3 && $tabs)) {
								$submenu = true;
							}
...
$(document).ready(function() {
	...
	$('a.menu').click(function(e){
		...
		$.ajax({
			type: "POST",
			url: '<?php echo ROOT . "/imageMaker/display_pdfs.php";?>', // absolute path
			data: {data:data}, 
			dataType: "html",
